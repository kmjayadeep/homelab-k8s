...
Qwen3-Coder-30B-A3B-Instruct has no native MTP head. `draft-mtp` cannot be enabled by flags alone; a separate draft model is a different mechanism and would use more memory. A future Qwen3.6-35B-A3B MTP GGUF trial is a **new model evaluation**, not an MTP upgrade of this checkpoint. A separate, pinned Qwen3.6-35B-A3B UD-IQ3_S + MTP trial released the sole B60 by pausing the coder workload without deleting its PVC; the coder endpoint was interrupted during the trial. llama.cpp SYCL build 11151 served 16K through 128K in 16K increments. At 128K, a 126,022-token prompt had 186.5 s TTFT, 677 tok/s prefill, 28.8 tok/s decode, and 17,125 MiB peak sampled VRAM (70.0%); the node stayed Ready without MemoryPressure. MTP was enabled, but an MTP-off baseline and coding/tool quality were **not** tested. The coder endpoint was restored after the trial; retain the trial and existing model caches for rollback and further comparisons. See [the evaluation](../plans/0005-intel-arc-b60-llm-serving-evaluation.md) for the complete step-by-step measurements. These results do not make the earlier Coder 128K Q8 configuration safe. This ADR records the earlier Qwen3.8 decision and trial rationale; see [the evaluation](../plans/0005-intel-arc-b60-llm-serving-evaluation.md) for detailed measurements and the proposed candidate.
